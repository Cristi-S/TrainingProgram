    ...
    lsAddbefore:
      begin
        ...
        begin
          ...
          if (List.SearchItem = List.GetFirst) and (temp = List.GetFirst)
          then ListControlItem.PaddingLeft := ListControlItem.PaddingLeft +
            +ListControlItem.ArrowWidth;

          // добавление в середину перед заданным
          next := temp.GetNext; if Assigned(next) then if temp.GetNext.IsAddBefore
          then begin NewListControlItem := TListControl.Create(FlowPanel1);
          NewListControlItem.ItemMain.TitleMain := List.NewItem.GetInfo;
          NewListControlItem.ItemMain.TitleNext := List.NewItem.GetNextInfo;
          NewListControlItem.ItemMain.TitlePrev := List.NewItem.GetPrevInfo;
          NewListControlItem.State := new;
          ...
          if temp.GetNext <> nil then begin NewListControlItem.ItemMain.
            ArrowUpRight.visible := (temp.GetNext = List.NewItem.GetNext);
          NewListControlItem.ItemMain.ArrowDownRight.visible :=
          ...
          // длинные
          ListControlItem.ItemMain.ArrowLongLeft.visible := (temp.GetNext <>
            List.NewItem) and (temp.GetNext.GetPrev = temp);
          ListControlItem.ItemMain.ArrowLongRight.visible := (temp.GetNext <>
            List.NewItem) and (temp.GetNext <> nil); end;

          NewListControlItem.State := new; ListControlItem.State := addAfter;
          ListControl.Add(NewListControlItem); end;

          // закрашивание temp
          if temp = List.TempItem then begin ListControlItem.ItemMain.color :=
            clBlue; end;

          temp := temp.GetNext; end; end;
          lsAddAfter: begin temp := List.GetFirst;
          while temp <> nil do begin ListControlItem :=
            TListControl.Create(FlowPanel1);
          ListControlItem.ItemMain.TitleMain := temp.GetInfo;
          ListControlItem.ItemMain.TitleNext := temp.GetNextInfo;
          ListControlItem.ItemMain.TitlePrev := temp.GetPrevInfo;
          ListControlItem.ItemMain.ArrowHeader.visible := temp.IsFirst;
          ...
          ListControl.Add(ListControlItem);
          if temp.IsAddAfter then begin NewListControlItem :=
            TListControl.Create(FlowPanel1);
          NewListControlItem.ItemMain.TitleMain := List.NewItem.GetInfo;
          NewListControlItem.ItemMain.TitleNext := List.NewItem.GetNextInfo;
          NewListControlItem.ItemMain.TitlePrev := List.NewItem.GetPrevInfo;
          ...
          if temp.IsLast then begin ListControlItem.IsLast := false;
          NewListControlItem.State := normal;
          ListControlItem.ItemMain.ArrowRight.visible := (temp.GetNext <> nil);

          ListControlItem.ItemMain.ArrowLeft.visible := (List.NewItem.GetPrev <>
            nil); end else begin NewListControlItem.State := new;
          ListControlItem.State := addAfter; end;
          ...
          temp := temp.GetNext; end; end; lsDelete: begin temp := List.GetFirst;
          while temp <> nil do begin ListControlItem :=
            TListControl.Create(FlowPanel1);
          ListControlItem.ItemMain.TitleMain := temp.GetInfo;
          ListControlItem.ItemMain.TitleNext := temp.GetNextInfo;
          ...
          if temp.IsDelete then begin ListControlItem.ItemMain.color :=
            clRed; end;
          // закрашивание temp
          if temp = List.TempItem then begin ListControlItem.ItemMain.color :=
            clBlue; end;
          ...
          if Assigned(List.DeleteItem) then begin
          // если удаляем с начала списка (первый элемент)
          if List.DeleteItem = List.GetFirst then begin if temp = List.DeleteItem
          then if temp.GetNext.GetPrev <> temp
          then ListControlItem.ItemMain.ArrowLeft.cross.visible := true; end;
          // если удаляем с конца (последний элеменет)
          if List.DeleteItem.GetNext = nil then begin
          // крестик вправо
          if (List.DeleteItem.GetPrev = temp) and (temp.GetNext <>
            List.DeleteItem) then begin ListControlItem.ItemMain.ArrowRight.
            cross.visible := true; end; end else
          // удаление из середины
          begin
          // длинная стрелочка вперед + крестик
          if (List.DeleteItem.GetPrev = temp) and (temp.GetNext <>
            List.DeleteItem) then begin ListControlItem.ItemMain.
            ArrowRightPolygon.visible := true;
          ListControlItem.ItemMain.ArrowRight.cross.visible := true; end;
          // длинная стрелочка назад
          if (List.DeleteItem.GetNext = temp.GetNext) and (temp.GetNext <>
            List.DeleteItem) and (temp <> List.DeleteItem) and
            (temp.GetNext.GetPrev = temp)
          then begin ListControlItem.ItemMain.ArrowLeftPolygon.visible :=
            true; end;
          // крестик
          if (temp = List.DeleteItem) and
            (List.DeleteItem.GetNext.GetPrev = List.DeleteItem.GetPrev)
          then begin ListControlItem.ItemMain.ArrowLeft.cross.visible := true;
          end; end; end;

          // обработка прохода по списку с учетом удаляемного элемента
          if Assigned(List.DeleteItem) then begin if List.DeleteItem.GetPrev = temp
          then temp := List.DeleteItem else temp := temp.GetNext;
          end else temp := temp.GetNext; end; end; end; ReplaceControlItems;
          UpdateButtonState; end;
          ...
          begin if not(Sender is TList) then Exit; RedrawPanel; end;

            procedure TForm1.ButtonCreateClick(Sender: TObject);
          var ListItem: TListItem; i, Count: integer; begin
          ...
          if RadioButton1.Checked = true then for i :=
            0 to Count do begin if i = 0 then begin ListItem :=
            TListItem.Create('item' + IntToStr(i));
          List.addAfter('item', ListItem); WaitForSingleObject(List.ThreadId,
            INFINITE); end else begin ListItem := TListItem.Create('item' +
            IntToStr(i)); List.addAfter('item' + IntToStr(i - 1), ListItem);
          WaitForSingleObject(List.ThreadId, INFINITE); end; end;
          // перерисовываем панель
          RedrawPanel();
          // возвращаем программу в режим управления
          Logger.Enabled := true; List.Mode := lmControl; end;

          // удаление
            procedure TForm1.ButtonDeleteClick(Sender: TObject);
          var SearchItem: string; begin SearchItem := InputBox('Удаление',
            'Введите элемент, который нужно удалить :', 'item1');
          List.Delete(SearchItem); end;

            procedure TForm1.ButtonAppendClick(Sender: TObject);
          var temp: TListItem; begin if List.Getcount = 0 then begin temp :=
            TListItem.Create('item' + IntToStr(List.Getcount));
          List.addAfter('', temp);
          end else begin temp := TListItem.Create('item' +
            IntToStr(List.Getcount));
          List.addAfter('item' + IntToStr(List.Getcount - 1), temp); end; end;

          // добавление первого элемента в список
            procedure TForm1.ButtonAddClick(Sender: TObject);
          var ListItem: TListItem; info: string; begin Form2.ShowModal;
          info := Form2.Edit1.Text; ListItem := TListItem.Create(info);
          List.addAfter('', ListItem); end;

          // возобновление работы
            procedure TForm1.ButtonNextClick(Sender: TObject);
          begin List.NextStep; end;

            procedure TForm1.ButtonRefreshClick(Sender: TObject);
          begin ButtonClearClick(Sender);
          // FlowPanel1.Components.DestroyComponents;
          { TODO пересмоттреть очистку списка, т.к. панель не очищается }
          ReplaceControlItems(); end;

            procedure TForm1.ButtonInsertClick(Sender: TObject);
          var temp: TListItem; SearchItem: string;
          begin if List.Getcount = 0 then begin temp := TListItem.Create('item'
            + IntToStr(List.Getcount)); List.addAfter('', temp);
          end else begin SearchItem := InputBox('Новый элемент',
            'Введите элемент, после которого добавить новый :', 'item1');
          temp := TListItem.Create('itemNew');
          List.addAfter(SearchItem, temp); end; end;

          // добавление после
            procedure TForm1.ButtonAddAfterClick(Sender: TObject);
          var ListItem: TListItem; SearchItem: string; info: string;
          begin if List.Getcount <> 0 then Form2.ShowModal;
          info := Form2.Edit1.Text; ListItem := TListItem.Create(info);
          SearchItem := InputBox('Добавление после заданного',
            'Введите элемент, после которого добавить новый :', 'item1');
          List.addAfter(SearchItem, ListItem); end;
          ...
          List.AddBefore(SearchItem, ListItem); end;

            procedure TForm1.ButtonClearClick(Sender: TObject); var i: integer;
          begin for i := 1 to FlowPanel1.ComponentCount do FlowPanel1.Controls
            [0].DisposeOf;

          for i := 1 to ListControl.Count do begin ListControl.Delete(0); end;
            ...
            var
            Key:
              Char); begin If not CharInSet(Key, ['0' .. '9', #8]) then Key
              := #0; end;
            ...
            ListControl := TList<TListControl>.Create; UpdateButtonState; end;

            procedure TForm1.RadioButton1Click(Sender: TObject);
            begin UpdateButtonState; end;
